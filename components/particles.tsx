...
export default function Particles({
  ...
  const circleParams = (): Circle => {
    ...
    const magnetism = 0.5 + Math.random() * 3;
    return {
      x,
      y,
      translateX: 0,
  ...
  const animate = () => {
    ...
    circles.current.forEach((circle: Circle, i: number) => {
      ...
      // 🌀 Mouse interaction: attract or repel near pointer
      const dx = circle.x - mouse.current.x - canvasSize.current.w / 2;
      const dy = circle.y - mouse.current.y - canvasSize.current.h / 2;
      const distance = Math.sqrt(dx * dx + dy * dy);
      const radius = 120; // mouse influence radius
      const force = remapValue(distance, 0, radius, 4, 0);

      if (distance < radius) {
        circle.translateX += (dx / distance) * force * circle.magnetism;
        circle.translateY += (dy / distance) * force * circle.magnetism;
      } else {
        circle.translateX += -circle.translateX / ease;
        circle.translateY += -circle.translateY / ease;
      }

      // Respawn particles that leave canvas
      if (
        circle.x < -circle.size ||
